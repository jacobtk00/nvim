local on_attach = function(client, buffer)
	...
	local keymaps = {
		...
		{ "[d",          "<cmd>lua vim.diagnostic.jump({ count = -1, float = true })<CR>",                                             desc = "Prev Diagnostic" },
		{ "[e",          "<cmd>lua vim.diagnostic.jump({ count = -1, float = true, severity = vim.diagnostic.severity.ERROR })<CR>",   desc = "Prev Error" },
		{ "[w",          "<cmd>lua vim.diagnostic.jump({ count = -1, float = true, severity = vim.diagnostic.severity.WARNING })<CR>", desc = "Prev Warning" },
		{ "]d",          "<cmd>lua vim.diagnostic.jump({ count = 1, float = true })<CR>",                                              desc = "Next Diagnostic" },
		{ "]e",          "<cmd>lua vim.diagnostic.jump({ count = 1, float = true, severity = vim.diagnostic.severity.ERROR })<CR>",    desc = "Next Error" },
		{ "]w",          "<cmd>lua vim.diagnostic.jump({ count = 1, float = true, severity = vim.diagnostic.severity.WARNING })<CR>",  desc = "Next Warning" },
		{ "g",           group = "goto" },
